...
class Actor(nn.Module):
    ...
    def __init__(self, state_dim, action_dim, hidden_size, init_w=3e-3):
        ...
        self.lstm1 = nn.LSTM(hidden_dim, hidden_size, 1, batch_first=True)
...
class SAC_LSTM:
    ...
    def __init__(self, **kwargs):
        ...
        self.buffer = ReplayBufferLSTM(buffer_size, "SAC_LSTM")
    ...
    def save(self):
        torch.save(self.Q_net.state_dict(), url + "SAC_LSTM_critic.pth")
        torch.save(self.Q_net_optimizer.state_dict(), url + "SAC_LSTM_critic_optimizer.pth")
        torch.save(self.actor.state_dict(), url + "SAC_LSTM_actor.pth")
        torch.save(self.actor_optimizer.state_dict(), url + "SAC_LSTM_actor_optimizer.pth")
        torch.save(self.log_alpha, url + "SAC_LSTM_log_alpha.pth")
        torch.save(self.log_alpha_optimizer.state_dict(), url + "SAC_LSTM_log_alpha_optimizer.pth")
        self.buffer.save()
    ...
    def load(self):
        ...
        if self.load_critic_flag == True:
            ...
            self.Q_net.load_state_dict(torch.load(url + "SAC_LSTM_critic.pth", map_location=device))
            self.Q_net_target = copy.deepcopy(self.Q_net)
        
        if self.load_log_alpha_flag == True:
            print("Load log-alpha.")
            self.log_alpha = torch.load(url + "SAC_LSTM_log_alpha.pth", map_location=device)
            self.log_alpha.requires_grad = True
            self.log_alpha_optimizer = optim.Adam([self.log_alpha], lr=alpha_lr)

        if self.load_actor_flag == True:
            print("Load actor model.")
            self.actor.load_state_dict(torch.load(url + "SAC_LSTM_actor.pth", map_location=device))
            self.actor_target = copy.deepcopy(self.actor)

        if self.load_optim_flag == True:
            print("Load optimizer.")
            self.Q_net_optimizer.load_state_dict(torch.load(url + "SAC_LSTM_critic_optimizer.pth", map_location=device))
            self.actor_optimizer.load_state_dict(torch.load(url + "SAC_LSTM_actor_optimizer.pth", map_location=device))
            self.log_alpha_optimizer.load_state_dict(torch.load(url + "SAC_LSTM_log_alpha_optimizer.pth", map_location=device))
